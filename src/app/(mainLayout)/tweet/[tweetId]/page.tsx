  ...
  if (!tweet) {
    return (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="mt-2 text-muted-foreground">This post may have been deleted.</p>
        </div>
    );
  }

  // Find parent tweet if this is a reply
  const parentTweet = tweet.parentId ? tweets.find((t) => t.id === tweet.parentId) : null;
      ...
      <div className="pb-16 md:pb-0">
        ...
        <article className="border-b border-border px-4 py-3">
          ...
          <div className="mt-3 flex items-center gap-1 text-sm text-muted-foreground">
            <span>
              {tweet.createdAt.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
            </span>
            <span>·</span>
            <span>
              {tweet.createdAt.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            </span>
            <span>·</span>
            <span className="font-bold text-foreground">{formatNumber(tweet.views)}</span>
            <span>Views</span>
          </div>
          ...
          <div className="mt-3 flex gap-4 border-t border-border pt-3 text-sm">
            <button className="hover:underline">
              <span className="font-bold text-foreground">{formatNumber(tweet.retweets)}</span>
              <span className="ml-1 text-muted-foreground">Reposts</span>
            </button>
            {quoteTweets.length > 0 && (
              <button className="hover:underline">
                <span className="font-bold text-foreground">{quoteTweets.length}</span>
                <span className="ml-1 text-muted-foreground">Quotes</span>
              </button>
            )}
            <button className="hover:underline">
              <span className="font-bold text-foreground">{formatNumber(tweet.likes)}</span>
